...
const FMTable: React.FC<StoreProps> = (props) => {
  ...
  const [fancyCategories, setFancyCategories] = useState<
  ...
  useEffect(() => {
    ...
    if (Array.isArray(fmData) && fmData.length > 0) {
      console.log(
        "[FMTable] Parent fmData detected - using parent data and skipping internal fetch",
      );
      try {
        // clone incoming fmData to avoid reference-equality issues
        const clonedFm = fmData.map((m) => ({ ...m }));
        setFancyData(clonedFm);
        if (tabVal === 0) setFilteredFancyMarketsData(clonedFm);
        // Sync categories if parent provided a mapping
        if (fancyCategoryMap && Object.keys(fancyCategoryMap).length > 0) {
          const categoriesArray = Object.entries(fancyCategoryMap).map(
            ([id, name]) => ({ id, name: String(name) }),
          );
          setFancyCategories(categoriesArray);
          const dynamicOrder = categoriesArray.map((fc) => ({
            fancyCategory: fc.name,
    ...
    const fetchFancy = async () => {
      ...
      console.log(
        "[FMTable] ✅ Fetching categories for eventId:",
        eventIdToUse,
      );
      ...
      try {
        ...
        if (response.status === 200 && response.data) {
          ...
          console.log(
            "[FMTable] Categories array length:",
          ...
          if (categoriesArray.length > 0) {
            setFancyCategories(categoriesArray);
            console.log(
              "[FMTable] ✅ Set fancyCategories state with",
              categoriesArray.length,
              "categories",
            );
            ...
            setDynamicCategoriesOrder(dynamicOrder);
            console.log(
              "[FMTable] ✅ Set dynamicCategoriesOrder with",
              dynamicOrder.length,
              "categories",
            );
          } else {
          ...
          if (response.data.data && Array.isArray(response.data.data)) {
            ...
            const transformedFancyData = response.data.data.map(
              (fancy: any) => {
                ...
                return {
                  ...
                  commissionEnabled:
                    fancy.is_commission_applied ||
                    fancy.commissionEnabled ||
                    false,
            ...
            // clone transformed data to ensure reactivity
            setFancyData(transformedFancyData.map((m) => ({ ...m })));
            console.log(
              "[FMTable] ✅ Set transformed fancyData with",
              transformedFancyData.length,
              "items",
            );
            console.log(
              "[FMTable] Sample transformed fancy market:",
              transformedFancyData[0],
            );
            ...
            if (tabVal === 0 && transformedFancyData.length > 0) {
              setFilteredFancyMarketsData(
                transformedFancyData.map((m) => ({ ...m })),
              );
              console.log(
                "[FMTable] ✅ Initialized filteredFancyMarketsData with all data for 'All' tab",
              );
            }
          } else {
    ...
    if (tabVal === 0) {
      ...
      if (allData.length > 0) {
        if (
          ...
          console.log(
            "[FMTable] ✅ Initialized filteredFancyMarketsData for 'All' tab with",
            allData.length,
            "items",
          );
        }
  ...
  useEffect(() => {
    if (Array.isArray(fmData) && fmData.length > 0) {
      try {
        console.log(
          "[FMTable] Deriving categories from parent fmData - items:",
          fmData.length,
        );
        const uniqueCats = Array.from(
          new Set(fmData.map((m) => String(m.category || "0").trim())),
        ).filter(Boolean);
                              ...
                              <span
                                ...
                                onClick={() => {
                                  ...
                                  const filtered = dataSource.filter((fm) => {
                                    const marketCategory = String(
                                      fm.category || "",
                                    ).trim();
                                    return (
                                      marketCategory === fc.id ||
                                      marketCategory === fc.name
                                    );
                                  });
                ...
                {(() => {
                  ...
                  if (tabVal === 0) {
                    ...
                    dataToDisplay =
                      safeFmData.length > 0 ? safeFmData : fancyData;
                    console.log(
                      "[FMTable] 'All' selected - showing all data:",
                    ...
                    dataToDisplay =
                      ...
                      filteredFancyMarketsData.length > 0
                        ...
                        : safeFmData.length > 0
                          ...
                          : fancyData;
                    console.log(
                      "[FMTable] Category selected - showing filtered data:",
                      dataToDisplay.length,
                    );
                  }
                  ...
                  console.log(
                    "[FMTable] Render check - safeFmData.length:",
                    safeFmData.length,
                  );
                  console.log(
                    "[FMTable] Render check - fancyData.length:",
                    fancyData.length,
                  );
                  console.log(
                    "[FMTable] Render check - filteredFancyMarketsData.length:",
                    filteredFancyMarketsData?.length || 0,
                  );
                  console.log(
                    "[FMTable] Render check - dataToDisplay.length:",
                    dataToDisplay.length,
                  );
                  ...
                  if (dataToDisplay.length > 0) {
                    ...
                    const uniqueCategories = Array.from(
                      new Set(
                        dataToDisplay
                          .map((fm) => String(fm.category || "").trim())
                          .filter(Boolean),
                      ),
                    );

                    console.log(
                      "[FMTable] Unique categories in dataToDisplay:",
                      uniqueCategories,
                    );
                    console.log("[FMTable] fancyCategories:", fancyCategories);
                    console.log(
                      "[FMTable] dynamicCategoriesOrder:",
                      dynamicCategoriesOrder,
                    );
                    ...
                    uniqueCategories.forEach((catName) => {
                      ...
                      const matchedCategory = fancyCategories.find(
                        (fc) =>
                          fc.name === catName ||
                          fc.id === catName ||
                          String(fc.id) === catName,
                      );
                      ...
                      categoryMap.set(catName, {
                        ...
                        order: matchedCategory
                          ? fancyCategories.indexOf(matchedCategory)
                          : 999, // Use index for ordering
                      });
                    });
                      ...
                      <>
                        ...
                        {categoryGroups
                          .filter((group) => {
                            ...
                            console.log(
                              `[FMTable] Group ${group.fancyCategory} has data:`,
                              hasData,
                            );
                            return hasData;
                          })
                            ...
                            <React.Fragment key={group.fancyCategory}>
                              ...
                              >
                                ...
                                {(() => {
                                  ...
                                  const categoryMarkets = dataToDisplay.filter(
                                    (fm) => {
                                      const marketCategory = String(
                                        fm.category || "",
                                      ).trim();
                                      return (
                                        marketCategory === group.fancyCategory
                                      );
                                    },
                                  );
                                  ...
                                  const sortedMarketsWithoutSeries =
                                    marketsWithoutSeries.sort((a, b) => {
                                      if (a?.sort - b?.sort != 0) {
                                        return a?.sort - b?.sort;
                                      }
                                      const aDesc = a.marketName;
                                  ...
                                  const sortedSeriesNames =
                                    Object.keys(seriesGroups).sort();

                                  return (
                                    <>
                                      {/* First: Show markets without series directly */}
                                      {sortedMarketsWithoutSeries.map(
                                        (fMarket, index) => {
                                          return !isFancyDisabled(
                                            fMarket.disable,
                                          ) ? (
                                            <React.Fragment
                                              key={`${group.fancyCategory}-no-series-${fMarket.marketId}-${index}`}
                                            >
                                              <FancyMarketRow
                                                eventData={eventData}
                                                fMarket={fMarket}
                                                ...
                                                showActiveBook={hasFancyLiability(
                                                  fMarket.marketId,
                                                )}
                                                minStake={
                                                  fMarket.isMarketLimitSet
                                                    ? fMarket?.marketLimits
                                                        ?.minStake
                                                      ? fMarket?.marketLimits
                                                          ?.minStake
                                                      : marketLimits[
                                      ...
                                      {sortedSeriesNames.map((seriesName) => {
                                        const seriesMarkets = seriesGroups[
                                          seriesName
                                        ].sort((a, b) => {
                                          if (a?.sort - b?.sort != 0) {
                                            return a?.sort - b?.sort;
                                        ...
                                        return (
                                          <React.Fragment
                                            key={`${group.fancyCategory}-series-${seriesName}`}
                                          >
                                            <Accordion
                                              defaultExpanded={true}
                                              ...
                                              {seriesMarkets.map(
                                                (fMarket, index) => {
                                                  return !isFancyDisabled(
                                                    ...
                                                    <React.Fragment
                                                      key={`${group.fancyCategory}-${seriesName}-${fMarket.marketId}-${index}`}
                                                    >
                                                      <FancyMarketRow
                                                        eventData={eventData}
                                                        fMarket={fMarket}
                        ...
                        <TableCell colSpan={3}>
                          <div className="fm-table-msg-text">
                            {langData?.["fancy_markets_not_found_txt"] ||
                              "No Fancy Markets for this event"}
                          </div>
                        </TableCell>
                      </TableRow>
                    );
...
const FancyMarketRow: React.FC<FancyMarketRowProps> = (props) => {
  ...
  return (
    <>
      <TableRow key={fMarket.marketId || "row-" + index}>
                ...
                if (
                  ...
                  const betData: PlaceBetRequest = {
                    ...
                    eventDate: eventData.openDate
                      ? typeof eventData.openDate === "string"
                        ? eventData.openDate
                        : new Date(eventData.openDate).toISOString()
                      : "",
        ...
        isFancySuspended(fMarket.suspend) === true ||
        isFancyDisabled(fMarket.disable) === true ? (
          <React.Fragment key={"row-" + index + "cell-5"}>
      ...
      {!oneClickBettingEnabled &&
      bets?.length > 0 &&
      String(bets?.[0]?.marketName ?? "") ===
        String(fMarket?.marketName ?? "") &&
      String(bets?.[0]?.marketId ?? "") === String(fMarket?.marketId ?? "") ? (
        <TableRow
...
const mapStateToProps = (state: RootState, ownProps?: Partial<StoreProps>) => {
  ...
  console.log(
    "[FMTable mapStateToProps] ownProps?.eventData:",
    ownProps?.eventData,
  );
