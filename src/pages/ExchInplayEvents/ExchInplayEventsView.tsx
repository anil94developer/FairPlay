...
const ExchInplayEventsView: React.FC<StoreProps> = (props) => {
  ...
  const [upcomingEventsFromAPI, setUpcomingEventsFromAPI] = useState<
    InplayEventsObj[]
  >([]);
  ...
  const getSportTabLabel = (sport: SportTabData): string => {
    const candidates = [sport?.langKey, sport?.text].filter(
      Boolean,
    ) as string[];
  ...
  const fetchUpcomingEvents = async () => {
    ...
    try {
      ...
      console.log("[ExchInplayEventsView] Upcoming events fetched:", {
        ...
        eventsBySport: upcomingEventsArray.map((s) => ({
          sport: s.sportName,
      ...
      console.error(
        "[ExchInplayEventsView] Error fetching upcoming events:",
        error,
      );
    } finally {
      setLoading(false);
    ...
    try {
      ...
      if (response?.data) {
        ...
        if (
          ...
          console.warn(
            "[ExchInplayEventsView] Invalid token detected, redirecting to login",
          );
      ...
      if (allMatches.length > 0) {
        ...
        inplayEvents.forEach((event: any) => {
          ...
          const sportName =
            event.sportName || event.sport_name || "Unknown Sport";
          ...
          const transformedRunners = (event.runners || []).map(
            (runner: any) => {
              const availableToBack = runner.ex?.availableToBack || [];
              const availableToLay = runner.ex?.availableToLay || [];
              ...
              return {
                runnerId: String(
                  runner.selectionId || runner.selection_id || "",
                ),
                runnerName:
                  runner.selection_name ||
                  runner.selectionName ||
          ...
          const eventDTO: EventDTO = {
            ...
            status: event.status || "UPCOMING",
            providerName:
              event.providerName || event.provider_name || "BetFair",
            homeTeam: homeTeam,
            awayTeam: awayTeam,
          ...
          if (sport.events.length < 3) {
            console.log(
              `[ExchInplayEventsView] Transformed event ${sport.events.length + 1}:`,
              {
                eventName,
                homeTeam,
                awayTeam,
        ...
        const transformedInplayEvents = Array.from(
          groupedEventsMap.values(),
        ).map((sport) => {
  ...
  const getEvents = () => {
    switch (statusNew) {
      ...
      case Status.UPCOMING:
        ...
        const eventsToUse =
          upcomingEventsFromAPI.length > 0
      ...
      case Status.SPORT:
        ...
        const allowedSportIds = sportIdMap[selectedSport || ""] || [
          selectedSport,
        ];
        return inplayEvents
          .filter((sport) => {
      ...
      <IonRow className="exch-inplay-events-view">
        ...
        >
          ...
          {(() => {
            ...
            const allInplayEvents =
              getEvents()?.flatMap((sport) => sport.events || []) || [];
